...
"""
    ...
    def __init__(
        self, url=SPECTRUM_URL, username=SPECTRUM_USERNAME, password=SPECTRUM_PASSWORD
    ):
        if url is None:
            raise ValueError(
                "Spectrum (OneClick) url must be provided either in the constructor or as an environment variable"
            )
    ...
    def _parse_update(self, res):
        ...
        if (
            root.find(".//ca:model", self.xml_namespace).get("error")
            == "PartialFailure"
        ):
    ...
    def _build_filter(self, filters, landscape=None):
        ...
        <and>
            ...
            landscape_filter=landscape_filter, filters=filters
        )
    ...
    def _check_http_response(res):
        ...
        if res.status_code == 401:
            raise SpectrumClientAuthException(
                "Authorization Failure. Invalid user name or password."
            )
    ...
    def search_models(self, xml):
        ...
        models = {
            model.get("mh"): {attr.get("id"): attr.text for attr in model.getchildren()}
    ...
    def generate_event_by_ip(self, event, address, variables):
        ...
        xml = self.event_by_ip_template.format(
            event=event, address=address, var_binds=var_binds
        )
